
//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

using Luban;


namespace ET
{
public sealed partial class MonsterTable : Luban.BeanBase
{
    public MonsterTable(ByteBuf _buf) 
    {
        Id = _buf.ReadInt();
        UnitId = _buf.ReadInt();
        Name = _buf.ReadString();
        Desc = _buf.ReadString();
        Level = _buf.ReadInt();
        HP = _buf.ReadInt();
        Attack = _buf.ReadInt();
        Defense = _buf.ReadInt();
        Speed = _buf.ReadFloat();
        AOI = _buf.ReadInt();
        AIConfigId = _buf.ReadInt();
        {int n0 = _buf.ReadSize(); Skills = new System.Collections.Generic.List<int>(n0);for(var i0 = 0 ; i0 < n0 ; i0++) { int _e0;  _e0 = _buf.ReadInt(); Skills.Add(_e0);}}
        DropId = _buf.ReadInt();
        Exp = _buf.ReadInt();
        IsBoss = _buf.ReadBool();
        Prefab = _buf.ReadString();
        MoveAsset = _buf.ReadString();
        JumpAsset = _buf.ReadString();
        Damage = _buf.ReadString();
        GetUp = _buf.ReadString();
        Death = _buf.ReadString();
        GroundToAir = _buf.ReadString();
        AirToGround = _buf.ReadString();
    }

    public static MonsterTable DeserializeMonsterTable(ByteBuf _buf)
    {
        return new MonsterTable(_buf);
    }

    /// <summary>
    /// Id
    /// </summary>
    public readonly int Id;
    /// <summary>
    /// Type
    /// </summary>
    public readonly int UnitId;
    /// <summary>
    /// 名字
    /// </summary>
    public readonly string Name;
    /// <summary>
    /// 描述
    /// </summary>
    public readonly string Desc;
    /// <summary>
    /// 等级
    /// </summary>
    public readonly int Level;
    /// <summary>
    /// 生命值
    /// </summary>
    public readonly int HP;
    /// <summary>
    /// 攻击
    /// </summary>
    public readonly int Attack;
    /// <summary>
    /// 防御
    /// </summary>
    public readonly int Defense;
    /// <summary>
    /// 移速
    /// </summary>
    public readonly float Speed;
    /// <summary>
    /// 视野（AOI）
    /// </summary>
    public readonly int AOI;
    /// <summary>
    /// AI配置Id
    /// </summary>
    public readonly int AIConfigId;
    /// <summary>
    /// 技能列表（|分隔）
    /// </summary>
    public readonly System.Collections.Generic.List<int> Skills;
    /// <summary>
    /// 掉落组
    /// </summary>
    public readonly int DropId;
    /// <summary>
    /// 经验
    /// </summary>
    public readonly int Exp;
    /// <summary>
    /// 是否是boss
    /// </summary>
    public readonly bool IsBoss;
    /// <summary>
    /// 模型预制体
    /// </summary>
    public readonly string Prefab;
    /// <summary>
    /// idle和移动动画资源
    /// </summary>
    public readonly string MoveAsset;
    /// <summary>
    /// 跳跃动画资源
    /// </summary>
    public readonly string JumpAsset;
    /// <summary>
    /// 受击动画
    /// </summary>
    public readonly string Damage;
    /// <summary>
    /// 起身动画
    /// </summary>
    public readonly string GetUp;
    /// <summary>
    /// 击倒活死亡
    /// </summary>
    public readonly string Death;
    /// <summary>
    /// 地面击飞
    /// </summary>
    public readonly string GroundToAir;
    /// <summary>
    /// 空中到地面
    /// </summary>
    public readonly string AirToGround;
   
    public const int __ID__ = 706228276;
    public override int GetTypeId() => __ID__;

    public  void ResolveRef(Tables tables)
    {
    }

    public override string ToString()
    {
        return "{ "
        + "Id:" + Id + ","
        + "UnitId:" + UnitId + ","
        + "Name:" + Name + ","
        + "Desc:" + Desc + ","
        + "Level:" + Level + ","
        + "HP:" + HP + ","
        + "Attack:" + Attack + ","
        + "Defense:" + Defense + ","
        + "Speed:" + Speed + ","
        + "AOI:" + AOI + ","
        + "AIConfigId:" + AIConfigId + ","
        + "Skills:" + Luban.StringUtil.CollectionToString(Skills) + ","
        + "DropId:" + DropId + ","
        + "Exp:" + Exp + ","
        + "IsBoss:" + IsBoss + ","
        + "Prefab:" + Prefab + ","
        + "MoveAsset:" + MoveAsset + ","
        + "JumpAsset:" + JumpAsset + ","
        + "Damage:" + Damage + ","
        + "GetUp:" + GetUp + ","
        + "Death:" + Death + ","
        + "GroundToAir:" + GroundToAir + ","
        + "AirToGround:" + AirToGround + ","
        + "}";
    }
}
}

