...
namespace ET
{
    ...
    public static partial class DBComponentSystem
    {
        ...
        {
            var mongoClientSettings = MongoClientSettings.FromConnectionString(dbConnection);

            // 优化超时设置（默认30秒太长，减少到5-10秒）
        ...
        /// <summary>
        /// 通过主键查询单个数据对象（高性能：直接使用_id索引）
        /// </summary>
        public static async ETTask<T> QueryById<T>(this DBComponent self, object id, string collection = null) where T : class
        {
            if (id == null)
            {
                return null;
            }
            long lockKey = id.GetHashCode();

            using (await self.Root().GetComponent<CoroutineLockComponent>().Wait(CoroutineLockType.DB, lockKey % DBComponent.TaskCount))
            {
                var filter = Builders<T>.Filter.Eq(MongoIdFieldName, id);
        ...
        /// <summary>
        /// 查询单个数据对象，指定taskId用于协程锁
        /// </summary>
        public static async ETTask<T> QuerySingle<T>(this DBComponent self, long taskId, Expression<Func<T, bool>> filter, string collection = null) where T : class
        {
        ...
        /// 查询多个数据对象
        /// </summary>
        public static async ETTask<List<T>> Query<T>(this DBComponent self, Expression<Func<T, bool>> filter, string collection = null) where T : class
        {
            using (await self.Root().GetComponent<CoroutineLockComponent>().Wait(CoroutineLockType.DB, RandomGenerator.RandInt64() % DBComponent.TaskCount))
            {
                using IAsyncCursor<T> cursor = await self.GetCollection<T>(collection).FindAsync(filter);
        ...
        /// <summary>
        /// 查询多个数据对象，指定taskId用于协程锁
        /// </summary>
        public static async ETTask<List<T>> Query<T>(this DBComponent self, long taskId, Expression<Func<T, bool>> filter, string collection = null) where T : class
        {
            using (await self.Root().GetComponent<CoroutineLockComponent>().Wait(CoroutineLockType.DB, taskId % DBComponent.TaskCount))
            {
                using IAsyncCursor<T> cursor = await self.GetCollection<T>(collection).FindAsync(filter);
        ...
        public static async ETTask<List<T>> QueryJson<T>(this DBComponent self, string json, string collection = null) where T : class
        {
            using (await self.Root().GetComponent<CoroutineLockComponent>().Wait(CoroutineLockType.DB, RandomGenerator.RandInt64() % DBComponent.TaskCount))
            {
                ...
                using IAsyncCursor<T> cursor = await self.GetCollection<T>(collection).FindAsync(filterDefinition);
                return await cursor.ToListAsync();
            }
        }
