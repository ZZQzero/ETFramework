    ...
    public class NetComponentOnReadInvoker_Gate: AInvokeHandler<NetComponentOnRead>
    {
        ...
        private async ETTask HandleAsync(NetComponentOnRead args)
        {
                ...
                switch (message)
            {
                ...
                {
                    long unitId = session.GetComponent<SessionUserEntityComponent>().UserEntity.CurrentRoleId;
            ...
            {
                Log.Error($"NetComponentOnReadInvoker_Gate 处理消息异常: {args.Message?.GetType().FullName ?? "null"}\n{e}");
            }
        }
    }
}